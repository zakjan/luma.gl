...
export default class IBLEnvironment {
  ...
  getDiffuseEnvSampler() {
    if (!this._DiffuseEnvSampler) {
      this._DiffuseEnvSampler = this.makeCube({
        ...
        getTextureForFace: dir => load(this.getTexUrl('diffuse', dir, 0), ImageLoader, options),
        parameters: {
      ...
      this._SpecularEnvSampler = this.makeCube({
        ...
        getTextureForFace: dir => {
          ...
          for (let lod = 0; lod <= this.specularMipLevels - 1; lod++) {
            imageArray.push(load(this.getTexUrl('specular', dir, lod), ImageLoader, options));
          }
          return imageArray;
        },
        parameters: {
  ...
  getBrdfTexture() {
    if (!this._BrdfTexture) {
      this._BrdfTexture = new Texture2D(this.gl, {
        ...
        data: load(this.brdfLutUrl, ImageLoader, options)
      });
    }
